...
#include "./ssim.h"
#ifdef HAVE_JPEG
#include "codec.camera/compare.h"
#include "codec.camera/convert.h"
#endif
...
bool ExtractResolutionFromFilename(const char* name,
  ...
  if (bytes_org == total_size) {
    if (0 == codec.camera::MJPGSize(ch_org, total_size, width_ptr, height_ptr)) {
      delete[] ch_org;
      return true;
    }
  }
...
bool UpdateMetrics(uint8_t* ch_org,
  ...
  if (do_psnr) {
    ...
    double y_err = static_cast<double>(
        codec.camera::ComputeSumSquareError(ch_org, ch_rec, y_size));
    double u_err = static_cast<double>(
        codec.camera::ComputeSumSquareError(u_org, u_rec, uv_size));
    double v_err = static_cast<double>(
        codec.camera::ComputeSumSquareError(v_org, v_rec, uv_size));
#else
...
int main(int argc, const char* argv[]) {
  ...
  for (number_of_frames = 0;; ++number_of_frames) {
    ...
    if (bytes_org < total_size) {
      ...
      if (0 != codec.camera::MJPGToI420(ch_jpeg, bytes_org, ch_org, image_width,
    ...
    for (int cur_rec = 0; cur_rec < num_rec; ++cur_rec) {
      ...
      if (bytes_rec < total_size) {
        ...
        if (0 != codec.camera::MJPGToI420(ch_jpeg, bytes_rec, ch_rec, image_width,
